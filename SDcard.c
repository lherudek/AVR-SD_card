...
unsigned char SD_init(void)
{
	...
	//SPI init
	SPCR = (1<<SPE)|(1<<MSTR)/*|(SPR1)*/;//Enable SPI, Master, set clock rate fck/4
...
unsigned char SD_readSingleBlock(unsigned long startBlock)
{
	...
	for(i=0; i<512; i++)	buffer[i] = SPI_receive();//read 512 bytes
...
unsigned char SD_writeSingleBlock(unsigned long startBlock)
{
	...
	for(i=0; i<512; i++)	SPI_transmit(buffer[i]);//send 512 bytes data
					...
					if((flag == GET_FILE) || (flag == DELETE))
					{
						...
						if(j == 11)
						{
							...
							else//when flag = DELETE
							{
								...
								if(firstCluster < cluster)	getSetFreeCluster (NEXT_FREE, SET, firstCluster);
...
{
	unsigned char j, data, fileCreatedFlag = 0, start = 0, appendFile = 0, sectorEndFlag = 0, sector;
	...
	if(j == 1) 
	{
		...
		while(1)
		{
			...
			if(nextCluster == END_OF_FILE) break;
			cluster = nextCluster;
			clusterCount++;
		}
	...
	else
	{
		...
		getSetNextCluster(cluster, SET, END_OF_FILE);   //last cluster of the file, marked EOF
	...
	while(1)
	{
		...
		{
			if(cluster == END_OF_FILE)   //this situation will come when total files in root is multiple of (32*sectorPerCluster)
			{  
				cluster = searchNextFreeCluster(prevCluster); //find next cluster for root directory entries
				getSetNextCluster(prevCluster, SET, cluster); //link the new cluster of root to the previous cluster
				getSetNextCluster(cluster, SET, END_OF_FILE);  //set the new cluster as end of the root directory
			} 
...
//Arguments: none
//return: none
//Note: this routine can take up to 15sec for 1GB card (@1MHz clock)
...
void memoryStatistics(void)
{
	...
	if(freeClusters > totalClusters)
	{
		...
		while(1)
		{
			...
			{
				value = (unsigned long *) &buffer[i*4];
				if(((*value)& 0x0fffffff) == 0)	freeClusters++;
...
{
	if(convertFileName(fileName)) return;
